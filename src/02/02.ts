...
const Me: { [K in MyChoices]: Outcome } = {
    X: 'Lose',
    Y: 'Draw',
    Z: 'Win',
};

const WinLossMap: { [K in Outcome]: { [J in RPS]: RPS } } = {
    Win: {
        Rock: 'Paper',
        Paper: 'Scissors',
        Scissors: 'Rock',
...
const requiredOutcome = (selection: RPS, outcome: Outcome): RPS => {
    if (outcome === 'Draw') return selection;

    return WinLossMap[outcome][selection];
};
...
try {
    ...
    for (let index = 0; index < gameList.length; index++) {
        ...
        if (!value) {
            ...
            const outcomeRequired = Me[myChoice];
            const theirRPS = Opponent[theirChoice];

            const myRPS = requiredOutcome(theirRPS, outcomeRequired);
